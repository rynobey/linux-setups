...
exec_always --no-startup-id sh -c "command -v xrdb >/dev/null && xrdb -merge ~/.Xresources"
...
# Avoid Tab in kb-row-down — collides with rofi's default kb-mode-next binding
# and produces a loader warning.
DOCKER_HELPER="$HOME/.local/bin/rofi-pubuntu-docker"
write_with_backup "$HOME/.config/rofi/config.rasi" <<EOF
configuration {
    modi:        "drun,run,window,docker:${DOCKER_HELPER}";
    combi-modi:  "drun,window,docker:${DOCKER_HELPER}";
    show-icons:   true;
    icon-theme:  "Adwaita";
    display-drun:   " Apps";
    display-run:    " Run";
    ...
    display-combi:  " All";
    /* On-screen-keyboard users often miss arrows — Ctrl+j/n / Ctrl+k/p cycle.
     * (Tab can't be re-bound here without colliding with kb-mode-next.) */
